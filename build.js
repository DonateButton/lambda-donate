...
const hasBigUpdate  = array => _.includes(array, '--update');
const hasSmallUpdate = array => _.includes(array, '-u');
const hasUpdate = array => hasBigUpdate(array) || hasSmallUpdate(array);
...
const performActionIfPassed = (callback)=>
{
    ...
    else if(program.action === ACTION_UPDATE)
    {
        updateFunction(lambda, callback);
    }
};
...
const updateFunction = (lambda, callback)=>
{
    const params = {
        FunctionName: FUNCTION_NAME,
        ZipFile: fs.readFileSync('deploy.zip')
